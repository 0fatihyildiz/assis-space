...
const RenderRowExpanded: FC<{ rowData: any }> = ({ rowData }) => {
  return (
    <div className="w-full relative cursor-default">
      {rowData.questions.map((item: any, i: number) => (
        <div
          key={`rowExpand-${i}`}
          className={
            "flex items-center justify-between px-2" + (i > 0 ? " mt-1" : "")
          }
        >
          ...
          <div className="relative flex border-[1px] border-base-gray rounded-md">
            ...
            <div
              className={
                ...
                (item.state === "correct" ? "bg-green-200" : "")
              }
            >
              <svg
            ...
            <div
              className={
                "p-2 text-red-400 hover:text-red-600 " +
                (item.state === "wrong" ? "bg-red-200" : "")
              }
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
...
export default function Users() {
  ...
  return (
    <div className="page relative flex-1 pb-6 px-6 lg:px-12 pt-20 mt-2 xl:pt-6 xl:mt-0 select-none">
      ...
      {isOpenProgress && (
        <Modal
          title={`Kullanıcı İlerlemesi - #${shownUser.id}`}
          btnText="Kapat"
          closeModal={() => setIsOpenProgress(false)}
          saveChanges={() => setIsOpenProgress(false)}
        >
          <Table
            rowKey={"expand"}
            ...
            renderRowExpanded={(rowData: any) => (
              <RenderRowExpanded rowData={rowData} />
            )}
          ></Table>
        </Modal>
      )}
        ...
        >
          <div className="flex flex-col md:flex-row">
            <div className="flex flex-col md:mr-2">
              <label
                htmlFor="key"
                className="text-xs font-semibold mb-2 mt-2 md:mt-0"
              >
                Ad
              </label>
              <input
            ...
            <div className="flex flex-col md:w-6/12 md:ml-2">
              ...
              >
                <input
                  disabled
                  type="checkbox"
                  checked={newItem.displayProfile}
                />
                <span className="slider round"></span>
              </label>
            </div>
      ...
      <div
        ...
        className="fixed bottom-0 right-0 xl:right-96 p-3 m-3 bg-base-color hover:bg-base-dark text-gray-50 rounded-full cursor-pointer transition-colors duration-2"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="h-5 w-5"
